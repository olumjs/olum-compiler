...
class WebSocket extends EventEmitter {
  constructor(socket) {
    ...
    socket.on("error", err => {
      // only forward if someone listens — an unhandled 'error' on an EventEmitter
      // would throw and take down the whole process on any abrupt disconnect
  ...
  _sendClose(code, reason) {
    ...
    if (this._socket.writable && !this._socket.writableEnded) this._socket.write(encodeFrame(0x8, payload));
