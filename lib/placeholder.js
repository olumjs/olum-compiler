...
function parseProps(attrsStr, parentMethods, propsAliases) {
  ...
  while ((match = attrRegex.exec(attrsStr)) !== null) {
    ...
    if (isExpr) {
      // FUNCTION PROP: a bare identifier naming a top-level function of THIS (parent) component,
      // e.g. toggle="{toggle}". Functions can't cross the JSON data-o-props channel, so skip the
      // JSON entry and record it in data-o-props-src as kind "method" — buildTree resolves it
      // live from the parent instance's methodsRef on every render pass.
      const bare = expr.trim();
      if (parentMethods && parentMethods.includes(bare) && /^[A-Za-z_$][\w$]*$/.test(bare)) {
        srcEntries.push(propName + ':method:' + bare);
        continue;
      }
      // PROP FORWARD: a bare identifier destructured from props() in THIS (parent) component,
      // e.g. `const { onMessage } = props()` + onMessage="{onMessage}". Emit both the JSON entry
      // (carries serializable values) and a kind "props" src entry keyed by the ORIGINAL prop name
      // — buildTree falls back to the parent's own incomingProps when JSON dropped the value
      // (it was a function), so function props survive any nesting depth.
      if (propsAliases && Object.prototype.hasOwnProperty.call(propsAliases, bare) && /^[A-Za-z_$][\w$]*$/.test(bare)) {
      ...
      // props source is the wrapper call `props().X` (old proxy access `props.X` also accepted)
...
function buildPropsAttr(attrsStr, parentMethods, propsAliases) {
  const { entries, srcEntries } = parseProps(attrsStr, parentMethods, propsAliases);

  // emit the two attributes independently: a component may receive only function props
  // (src entries, no JSON payload) or only data props (JSON payload, no src entries)
  let attr = "";
  if (entries.length) {
...
function placeholder(content, regex, type, parentMethods, propsAliases) {
  return content.replace(regex, (str, $1, $2) => {
    ...
    if (isFullArr(data)) {
      ...
      if (isUpper(name[0])) {
        name = "name='" + name + "'";
        const propsAttr = data.length > 1 ? buildPropsAttr(data.slice(1).join(" "), parentMethods, propsAliases) : "";
