...
function parseProps(attrsStr, parentMethods) {
  const entries = [];
...
function buildPropsAttr(attrsStr, parentMethods) {
  const { entries, srcEntries } = parseProps(attrsStr, parentMethods);
...
function placeholder(content, regex, type, parentMethods) {
  return content.replace(regex, (str, $1, $2) => {
    ...
    if (isFullArr(data)) {
      ...
      if (isUpper(name[0])) {
        name = "name='" + name + "'";
        const propsAttr = data.length > 1 ? buildPropsAttr(data.slice(1).join(" "), parentMethods) : "";
