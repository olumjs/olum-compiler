...
const RUNTIME_IMPORT = `import __olumPkg from "olum";const olum=__olumPkg.__olum||window.olum;`;
...
const parser = (str, counter, filePath) => {
  ...
  const head = headParser(sfc.head, filePath);
  ...
  if (process.env.NODE_ENV === "development") {
    try {
      ...
      (comps || []).forEach((name) => {
        ...
        spec = resolveAlias(spec, filePath);
  ...
  if (imports)
    imports = imports.map((item) =>
      splitBarrels(toJs(item.statement, filePath)),
    );
  ...
  return {
    ...
    stateProps: ${hasState ? "state" : "null"},
