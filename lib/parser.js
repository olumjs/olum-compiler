...
const { isFullArr, id, createScopeHandler, getGlobalMethods, getGlobalProps, minifyJS } = require("./helpers");
...
const parser = (str, counter, filePath) => {
  ...
  const methodsTobeExcluded = ["mounted", "unMounted"];
  const propsTobeExcluded = ["watcher"];
  ...
  return `${breaks}${imports ? imports.join("\n") + "\n" : ""}export default (_instanceKey) => {const _storeKey=_instanceKey||"${compName}";${minifyJS(propsProxy)}${js}
  ...
  return {
    ...
    hooks: {
      mounted: ${hasMounted ? "mounted" : "null"},
      unMounted: ${hasUnMounted ? "unMounted" : "null"},
      isMounted: false,
      isUnMounted: false,
    }
  };
