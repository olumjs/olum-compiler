...
function renameKeyframes(css, compName) {
  ...
  names.forEach((name) => {
    ...
    css = css.replace(
      new RegExp(`(@keyframes\\s+)${re}\\b`, "g"),
      "$1" + newName,
    );
    css = css.replace(
      new RegExp(`(animation-name\\s*:[^;{]*)\\b${re}\\b`, "g"),
      "$1" + newName,
    );
    css = css.replace(
      new RegExp(`(animation\\s*:[^;{]*)\\b${re}\\b`, "g"),
      "$1" + newName,
    );
  });
  return css;
  ...
  function scopeSelectors(sel) {
    return sel
      ...
      .map((s) => {
        ...
        return (
          s.slice(0, start) + compound.slice(0, k) + scope + compound.slice(k)
        );
      })
      .filter(Boolean)
      .join(", ");
...
function cssParser(content, scopeAttr, compName) {
  content = (content || "").trim();
  if (scopeAttr && compName && content)
    content = scopeCSS(content, scopeAttr, compName);
