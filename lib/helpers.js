...
${routes
  .map(({ path, comp }) => {
    paths.push(`${cleanNumbersPrefix(path, "url")}`);
    return `  { path: "${cleanNumbersPrefix(path, "url")}", comp: ${cleanNumbersPrefix(comp)} }`;
  })
  .join(",\n")}
];`;

  fs.writeFileSync("./routes.json", `${JSON.stringify(paths, null, 2)}`);
  return result;
}
