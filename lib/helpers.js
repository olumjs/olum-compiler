...
const cleanParentheses = (filePath, type="name") => {
  ...
  } else if (type == "url"){ // clean slash after it also e.g. (main)/
    return filePath.replace(/\([^)]*\)\//g, "")
  }
  return filePath;
}
...
const cleanNumbersPrefix = (name, type = "name") => {
  if (type === "url") {
    // only the NN- ordering prefix at a segment start — a blanket digit strip would
    // corrupt legit digit segments ("/404" -> "/", "/7guis" -> "/guis")
  ...
  */
  ...
  function shareAllProps(globalNamesArr) {
    ...
    if (hasState && isFullArr(stateProps)) {
      ...
      arr.push(`get state(){return window.olum.proxyHandlerForScope(${"{" + fragments.join(",") + "}"}, state);}`);
    }
    return arr;
  }
...
// top-level bindings destructured from props(): const { onMessage, onSave: save } = props()
// returns a localName -> propKey map so placeholder.js can flag bare-identifier prop forwards
// (onMessage="{onMessage}") as kind "props" src entries — functions can't cross the JSON channel
function getPropsAliases(str) {
  const aliases = {};
  acorn.parse(str, { ecmaVersion: "latest", sourceType: "module" }).body.forEach((obj) => {
    if (obj.type !== "VariableDeclaration" || !Array.isArray(obj.declarations)) return;
    obj.declarations.forEach((decl) => {
      if (
        ...
        decl.id.properties.forEach((prop) => {
          if (prop.type === "Property" && prop.key && prop.key.name && prop.value && prop.value.type === "Identifier") {
            aliases[prop.value.name] = prop.key.name; // localName -> original prop key (handles renames)
          }
        });
      }
    });
...
function generateImports(manifest) {
  return Object.values(manifest)
    .map(({ page }) => {
      const component = cleanNumbersPrefix(toComponentName(page));
      return `import ${component} from "./${page.replace(/.html$/, ".js")}";`;
    })
    .join("\n");
}
...
${routes
  .map(({ path, comp }) => `  { path: "${cleanNumbersPrefix(path, "url")}", comp: ${cleanNumbersPrefix(comp)} }`)
  .join(",\n")}
];`;
}
...
const compileRoutes = (entryPoint) => {
  ...
  routes = routes.filter((item) => {
    ...
    const inExcludedDir = segments.some((seg) => ["components", "utils"].includes(seg) || seg.startsWith("_"));
