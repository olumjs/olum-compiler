...
const { RAW_OPEN, RAW_CLOSE, LT, GT, shieldTags, unshieldTags } = require("./tokens");
...
function htmlparser(str, counter, filePath, parentMethods, propsAliases) {
  ...
  // using dom parser — shieldTags renames parser-restricted containers (select/table/tr/…) to
  // unknown elements so a <for>/<if>/<show> inside them survives the parse; restored after serialize
  const body = new jsdom.JSDOM(shieldTags(content)).window.document.body;
  handleHtml(body); // html="expr" -> unescaped ${expr} content (before control-flow dissolves elements)
  handleTextarea(body); // textarea value="..." -> text content (no value attribute exists on textarea)
  ...
  content = content.replace(/\{([^{}]+)\}/g, (match, expr) => {
    ...
    // the captured expr is JS, but if it sat in a TEXT node jsdom entity-encoded its <,>,& on
    // serialize — decode them back so comparisons like {width > 380 ? a : b} stay valid JS
