...
function htmlparser(str, counter, filePath, parentMethods) {
  let content = str.replace(script, "").replace(style, "");
  content = clean(content); // clean html comments to avoid runtime errors
  content = placeholder(content, selfClosingTag, "self", parentMethods);
  content = placeholder(content, normalTag, "normal", parentMethods);
  ...
  handleEvents(body, counter, filePath, anonHandlers);
