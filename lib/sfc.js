...
function extractHead(source, filePath) {
  const rawRanges = scanRawTextBlocks(source, ["script", "style"]).map(
    (block) => [block.start, block.end],
  );
  const inRawText = (at) =>
    rawRanges.some(([start, end]) => at >= start && at < end);
  const lineAt = (at) => source.slice(0, at).split("\n").length;

  const open = /<head(?=[\s/>])/gi;
  const blocks = [];
  let match;
  while ((match = open.exec(source)) !== null) {
    if (inRawText(match.index)) continue;
    const tagEnd = findTagEnd(source, match.index);
    if (tagEnd === -1) break;
    const close = source.slice(tagEnd + 1).match(/<\/head\s*>/i);
    if (!close) {
      warn(
        filePath,
        lineAt(match.index),
        "<head> is never closed, so it was ignored. Add a </head>.",
      );
      break;
    }
    const innerEnd = tagEnd + 1 + close.index;
    blocks.push({
      inner: source.slice(tagEnd + 1, innerEnd),
      start: match.index,
      end: innerEnd + close[0].length,
    });
    open.lastIndex = innerEnd + close[0].length;
  }
  if (!blocks.length) return { head: "", source };

  if (blocks.length > 1) {
    warn(
      filePath,
      lineAt(blocks[1].start),
      "A component compiles only its FIRST <head> — this one was dropped. Merge the tags into the first block.",
    );
  }
  ...
  blocks.forEach((block) => {
    out +=
      source.slice(cursor, block.start) +
      source.slice(block.start, block.end).replace(/[^\n]/g, "");
    cursor = block.end;
  });
  out += source.slice(cursor);

  return { head: blocks[0].inner, source: out };
}
...
function splitSFC(source, filePath) {
  ...
  const headBlock = extractHead(source, filePath);
  source = headBlock.source;
  ...
  const isDataScript = (node) =>
    node.attrs.some(
      (a) =>
        a.name.toLowerCase() === "type" &&
        a.value.trim().toLowerCase() === "application/ld+json",
    );

  const script = scripts.find((node) => !isDataScript(node)) || null;
